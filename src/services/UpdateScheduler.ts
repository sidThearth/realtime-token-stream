...
export class UpdateScheduler {
    ...
    private async pollUpdates() {
        ...
        try {
            ...
            // For demo purposes, let's just re-search "SOL" to get fresh data for the top tokens
            // This simulates a "market overview" update.
            const freshTokens = await this.tokenService.searchTokens('SOL', true);

            freshTokens.forEach(token => {
                // SIMULATION: Add larger random jitter (±5%) to demonstrate UI updates clearly
                // because API data might be cached or stable.
                const jitter = 1 + (Math.random() * 0.10 - 0.05);
                token.priceSol *= jitter;
                token.priceChange24h += (Math.random() * 2 - 1);
